...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if is_sub:
        ...
        welcome_text += (
            ...
            f"💰 Стоимость: <b>{config.SUBSCRIPTION_PRICE} ₽ / {config.SUBSCRIPTION_DAYS} дней</b>.\n\n"
...
async def show_payment_info(message: Message, state: FSMContext):
    ...
    payment_info = (
        f"💳 <b>{action_text}</b>\n\n"
        f"💰 Сумма к оплате: <b>{config.SUBSCRIPTION_PRICE} ₽</b>\n"
        f"⏳ Срок действия: <b>{config.SUBSCRIPTION_DAYS} дней</b>\n\n"
        "<b>Реквизиты для перевода:</b>\n"
        f"🏦 Банк: <b>{config.CARD_BANK}</b>\n"
...
async def cancel_callback(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    is_sub = await db.is_user_subscribed(callback.from_user.id)
...
async def process_receipt(message: Message, state: FSMContext, bot: Bot):
    ...
    admin_caption = (
        ...
        f"💰 <b>Сумма:</b> {config.SUBSCRIPTION_PRICE} ₽\n"
...
async def show_profile(message: Message):
    ...
    if is_sub and user.get("subscription_until"):
        sub_dt = datetime.fromisoformat(user["subscription_until"])
